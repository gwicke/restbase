...
function makeRequestTemplate(spec) {
    ...
    var reqEvaluator = new request_parser().eval_request
    reqEvaluator = reqEvaluator(spec);

    return function (restbase, req) {
        var newReq = reqEvaluator(req); 
 ...
 */
Router.prototype._handleSwaggerPathSpec = function(node, pathspec, operations) {
    var self = this;
    if (!pathspec) {
        ...
        loaderPromise = loaderPromise.then(function() {
            return Promise.each(subSpecs, function(subSpecOrPath) {
                ...
                .then(function(subSpec) {
                    return self._handleSwaggerSpec(node, subSpec, operations);
                });
            });
        });
    }
        ...
        loaderPromise = loaderPromise.then(function() {
            return Promise.each(restBaseModules, function(m) {
                ...
                .then(function(module) {
                    ...
                    return self._handleSwaggerSpec(node, module.spec, module.operations);
                });
            });
        });
    }
    ...
    function handlePaths (paths) {
        ...
        return Promise.all(Object.keys(paths).map(function(pathPattern) {
            ...
            if (!subtree) {
                ...
                specPromise = self._handleSwaggerPathSpec(subtree, pathSpec, operations);
            } else {
